...
def grade(input_data, quality_data, submission):    
    ...
    badVals = set()
    for v in lineTwoParts:
        if v < 0 or v > nodeCount-1:
            badVals.add(v)
    if len(badVals) > 0:
        return {'score':0.0, 'feedback':'the second output line should only contain values between 0 and '+str(nodeCount-1)+', this output has the following bad values: '+(', '.join(map(str,badVals)))}
    ...
    unused = set(range(0, nodeCount))
    for v in perm:
        unused.discard(v)
    if len(unused) > 0:
        return {'score':0.0, 'feedback':'the solution is not a permutation of the values 0..'+str(nodeCount-1)+', this solution is missing: '+(', '.join(map(str,unused)))}


    value = length(points[perm[-1]],points[perm[0]])
    for index in range(0, nodeCount-1):
        value += length(points[perm[index]],points[perm[index+1]])
